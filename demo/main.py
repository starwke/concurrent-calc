...
def generate_data(table_num: int, len_min: int, len_max: int, column_min: int, column_max: int, enum_min: int,
                  enum_max: int) -> list[dict]:
    if table_num <= 0:
    ...
    for _ in range(table_num):
        ...
        for _ in range(column_min if column_min == column_max else random.randint(column_min, column_max)):
            ...
            column_infos.append(
                {"data_format": data_type, "column_name": column_name, "column_enum_value": enum_values})
